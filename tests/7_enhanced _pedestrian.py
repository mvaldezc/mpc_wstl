...
def wstl_synthesis_control(
    ...
    for k in range(time_horizon):  
        ...
        delta_px[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-100, ub=100, name=name)
        name = "delta_py_{}".format(k)
        delta_py[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-5, ub=5, name=name)
        name = "delta_vx_{}".format(k)
        delta_vx[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-80, ub=80, name=name)
        ...
        x_ped[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-20, ub=80, name=name)
        name = "y_ped_{}".format(k)
        y_ped[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-2, ub=10, name=name)
        name = "x_dist_{}".format(k)
        x_dist[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-100, ub=100, name=name)
        name = "y_dist_{}".format(k)
        y_dist[k] = wstl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-5, ub=5, name=name)
...
if __name__ == '__main__':  
    ...
    T = 0.1 # sampling time
    timelength_random = 6 #+ 2*np.random.rand()
    horizon = int(timelength_random/T)+1
    ...
    #phi_confort = f"G[0,{horizon}]^w2 &&^p2(u_abs <= 10, j_abs <= 30)"
    ...
    vars_lb = {'px': -20, 'py': 0, 'vx': -80, 'vy': -20}
    vars_ub = {'px': 80, 'py': 5, 'vx': 80, 'vy': 20}
    control_lb = {'u_ax': -15, 'u_ay': -5, 'jx': -45, 'jy': -15}
    control_ub = {'u_ax': 15, 'u_ay': 5, 'jx': 45, 'jy': 15}

    # Define the initial and final conditions
    x_0 = {'px': 0, 'py': 1.25, 'vx': 0, 'vy': 0}
    x_f = {'px': 60, 'py': 1.25, 'vx': 0, 'vy': 0}

    # Define the pedestrian model
    class pedestrian:
        def __init__(self):
            self.x_ped = 48 #+ 5*np.random.rand()
            self.y_ped = 5.75
        def __call__(self, t):
            if t <= 5:
                vel = 6.5/5.0
                self.x_ped = self.x_ped #+ 0.1*np.random.randn(1)
                self.y_ped = 5.75 - vel*t
