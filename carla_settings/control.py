...
def read_trajectory_file(args):
    ...
    # Transformate coordinates, (0,2.5) should be (-143, -4), (1,2.5) should be (-144, -4)
    for i in range(len(traj_pos)):
        traj_pos[i] = carla.Location(x=-traj_pos[i].x - 143.0, y=traj_pos[i].y - 6.5, z=0.2)
...
def game_loop(args):
    ...
    try:
        ...
        # Set basic agent
        agent = BasicAgent(world.player, target_speed=36, opt_dict={
            'sampling_resolution': 0.25, 'ignore_traffic_light': True, 'max_brake': 1.0, 
            'longitudinal_control_dict' : {'K_P': 1.0, 'K_D': 0.00, 'K_I': 0.05, 'dt': 1.0/20.0}})
        agent.follow_speed_limits(False)
        
        # Create a toy trajectory, start (-143, -4), end (-259, -4), resolution 1.0, T = 0.2
        # traj_pos = []
        # start = (-143.0, -4.0, 0.0)
        # for i in range(0, 65):
        ...
        for i in range(1, len(traj_pos)):
            distance = traj_pos[i].distance(traj_pos[i-1])
            vel = distance / T
            traj_vel.append(vel)
        ...
        for location in traj_pos:
            # Set next position
            print("Location: ", location)
            agent.set_destination(location)
            # Set next velocity
            target_speed = traj_vel[traj_pos.index(location)] * 3.6
